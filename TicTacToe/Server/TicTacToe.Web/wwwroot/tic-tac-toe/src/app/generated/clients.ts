...
export class GameClient {
    ...
    add(cellType: CellType | undefined): Observable<string> {
        let url_ = this.baseUrl + "/Game";
        url_ = url_.replace(/[?&]$/, "");

        const content_ = new FormData();
        if (cellType === null || cellType === undefined)
            throw new Error("The parameter 'cellType' cannot be null.");
        else
            content_.append("cellType", cellType.toString());
    ...
    getMy(): Observable<GameModel[] | null> {
    ...
    protected processGetMy(response: HttpResponseBase): Observable<GameModel[] | null> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
